...
The PR Review workflow is a review-only workflow for evaluating pull requests. It follows a structured review process to fetch PR details, review spec compliance, review code quality, and compile findings by severity.

**Core principle:** Read-only git operations. No implementation or code changes.

**Key invariants:**
- PR must be fetched before review
...
- **Gate:** none
- **Triage:** Proceed if PR details and diff are successfully fetched

### Stage 1: Review Spec Compliance
- **Skill:** requesting-code-review
- **Input:** pr_details.md, diff.md
...
- PR cannot be fetched (invalid URL, permissions)
...
|-------|----------------------------------------|
| requesting-code-review | pr_url, pr_details.md, diff.md, spec_review.md, quality_review.md |
...
2. **Load present artifacts** to reconstruct reasoning train:
   - All produced artifacts (pr_details.md, diff.md, spec_review.md, etc.)
...
- PR review workflow is review-only
- No git write operations allowed
- Use `git fetch`, `git show`, `git diff` for review
...
- Fetch PR details (title, description, author, reviewers)
- Fetch diff (changed files, line changes)
- Document in pr_details.md and diff.md

### Stage 1: Review Spec Compliance
- Trigger phrase: "This is a compliance review task, code verification, artifact audit, and quality check." (swe-compliance for reviewer)
- Review PR against spec and requirements
- Check if PR addresses the intended changes
