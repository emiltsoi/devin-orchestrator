...
### Tasks
1. Create `devin_orchestrator/workflow_stage_executor.py` with a `WorkflowStageExecutor` class.
...
6. Update imports and unit tests in `devin_orchestrator/test_orchestration_engine.py` and `tests/`.

### Files touched
- `devin_orchestrator/orchestration_engine.py`
- `devin_orchestrator/workflow_stage_executor.py` (new)
- `devin_orchestrator/test_orchestration_engine.py`
...
- **File:** `devin_orchestrator/orchestration_engine.py` (or `WorkflowStageExecutor` after Phase 0)
...
- **File:** `devin_orchestrator/guardrails.py`
- **Tasks:**
...
- **File:** `mcp_server.py` (`_tool_dispatch_devin`), `devin_orchestrator/devin_cli_adapter.py`
...
- **File:** `devin_orchestrator/config_loader.py`
- **Tasks:**
...
### Files touched
- `devin_orchestrator/orchestration_engine.py`
- `devin_orchestrator/gate_controller.py` (new)
- `devin_orchestrator/artifact_validator.py` (new)
- `devin_orchestrator/triage_evaluator.py` (new)
- `devin_orchestrator/stage_skill_dispatcher.py` (new)
...
- **File:** `devin_orchestrator/orchestration_engine.py` / `GateController`
...
- **File:** `devin_orchestrator/floor_validator.py`
- **Tasks:**
...
- **File:** `devin_orchestrator/triage_evaluator.py` (new), `devin_orchestrator/guardrails.py`
...
- **File:** `devin_orchestrator/triage_evaluator.py`
...
- **File:** `mcp_server.py`, `devin_orchestrator/orchestration_engine.py`
...
- **Files:** `devin_orchestrator/orchestration_engine.py` / `GateController`
...
- **Files:** `devin_orchestrator/transport_adapter.py`, `devin_orchestrator/devin_cli_adapter.py`, `devin_orchestrator/skill_invoker.py`
- **Options:**
...
- **Tasks:**
  - Add `pyproject.toml` with package metadata, `devin_orchestrator` as a package, and console scripts.
  - Remove `sys.path.insert(0, "devin_orchestrator")` from `mcp_server.py` and the dispatch scripts.
...
- **Files:** `devin_orchestrator/metrics.py`, `devin_orchestrator/monitoring.py`, `devin_orchestrator/orchestration_engine.py`, `stateless_orchestrator.py`
...
- **Files:** `devin_orchestrator/config_loader.py` or `devin_orchestrator/security_utils.py`, `mcp_server.py`, `devin_orchestrator/skill_invoker.py`, `dispatch_skill.py`
...
- Run `devin_orchestrator/test_orchestration_engine.py` and `tests/` after every phase.
...
### Completed
- **Phase 0**: `WorkflowStageExecutor` extracted into `devin_orchestrator/workflow_stage_executor.py`; `OrchestrationEngine` delegates stage execution. `TriageDecision` moved and re-exported.
...
### Latest Test Results
- `devin_orchestrator/`: `462 passed, 1 skipped`
- `tests/`: `111 passed, 1 skipped`
- combined `devin_orchestrator/` + `tests/`: `573 passed, 2 skipped`

### Follow-up: Isolated Collaborator Unit Tests
- Added `devin_orchestrator/test_artifact_validator.py` (8 tests)
- Added `devin_orchestrator/test_triage_evaluator.py` (14 tests)
- Added `devin_orchestrator/test_stage_skill_dispatcher.py` (11 tests)
- Added `devin_orchestrator/test_gate_controller.py` (19 tests)
...
### Phase 2 Completion
- Extracted `ArtifactValidator` into `devin_orchestrator/artifact_validator.py`.
- Extracted `TriageEvaluator` into `devin_orchestrator/triage_evaluator.py` (also moved `TriageDecision` there).
- Extracted `StageSkillDispatcher` into `devin_orchestrator/stage_skill_dispatcher.py`.
- Extracted `GateController` into `devin_orchestrator/gate_controller.py` (also moved gate mode constants there).
...
### Test Results
- `devin_orchestrator/`: `531 passed, 1 skipped`
- `tests/`: `111 passed, 1 skipped`
- Combined `devin_orchestrator/` + `tests/`: `642 passed, 2 skipped`
...
### Added / updated tests
- `devin_orchestrator/test_stateless_orchestrator.py`: async dispatch tests for workflows, skills, continue, and execute routing.
- `devin_orchestrator/test_gate_controller.py` and `devin_orchestrator/test_orchestration_engine.py`: updated to mock `wait_for_file_change` instead of `time.sleep` for gate/pause scenarios.

### Test Results
- `devin_orchestrator/`: `529 passed, 2 skipped`
- `tests/`: `111 passed`
- Combined `devin_orchestrator/` + `tests/`: `640 passed, 2 skipped`


## Phase 4 Review Fixes (2026-07-28)

### Completed
- **Packaging completeness**: Added `devin_orchestrator/__init__.py` and configured `pyproject.toml` to package `devin_orchestrator/` as `devin_orchestrator` using `package-dir`. Root entry-point scripts (`mcp_server.py`, `dispatch_devin.py`, `dispatch_skill.py`) now fall back to the installed `devin_orchestrator` package when `devin_orchestrator/` is not a sibling directory. Added `requests` to runtime dependencies.
...
### Test Results
- Combined `devin_orchestrator/` + `tests/`: `640 passed, 2 skipped`, 0 warnings.
