...
from deterministic_tools import load_manifest, load_skill
...
class TestWorkflowExecutionIntegration(unittest.TestCase):
    ...
    def _mock_invoke_with_artifacts(self, skill_name, context, workspace, **kwargs):
        """Helper to mock invoke_skill and create expected output artifacts"""
        session_dir = Path(workspace)
        for stage in self.workflow_manifest["stages"]:
            if stage["skill"] == skill_name:
                for artifact in stage.get("output_artifacts", []):
                    (session_dir / artifact).write_text(
                        f"# {artifact} output\n\nGenerated for test.", encoding="utf-8"
                    )
        return SkillInvocationResult(
            success=True,
            session_id="skill-{}".format(context.get("session_id", "unknown")),
            output=f"{skill_name} executed successfully",
            error=None,
        )
    ...
    def test_complete_workflow_execution_success(self):
        ...
        # Mock skill invoker to return success and create artifacts
        with patch(
            "orchestration_engine.load_skill", side_effect=self._mock_load_skill
        ):
            with patch.object(
                self.engine.skill_invoker,
                "invoke_skill",
                side_effect=self._mock_invoke_with_artifacts,
            ):
                # Execute workflow
    ...
    def test_skill_invocation_through_orchestration(self):
        ...
        with patch(
            ...
            with patch.object(
                ...
                # Collect skill names from keyword call arguments, excluding reviewer dispatches
                invoked_skills = [
                    call.kwargs["skill_name"]
                    for call in mock_invoke.call_args_list
                    if call.kwargs.get("skill_name") != "swe-compliance"
                ]
    ...
    def test_gate_decision_flow_approve(self):
        ...
        # Mock skill invoker to create artifact and return success
        def mock_invoke_with_design(skill_name, context, workspace, **kwargs):
            (Path(workspace) / "design.md").write_text(
                "# Design\n\nValid design.", encoding="utf-8"
            )
            return SkillInvocationResult(
                success=True,
                session_id=f"skill-{session_id}",
        ...
        with patch(
            ...
            with patch.object(
                self.engine.skill_invoker,
                "invoke_skill",
                side_effect=mock_invoke_with_design,
            ) as mock_invoke:
    ...
    def test_retry_loop_with_actual_corrections(self):
        ...
        def mock_invoke_skill(
            skill_name, context, workspace, correction_artifact=None, **kwargs
        ):
            if skill_name == "swe-compliance":
                # Reviewer dispatch - return approval
                return SkillInvocationResult(
                    success=True,
                    session_id=f"reviewer-{session_id}",
                    output="Reviewer approved",
                    error=None,
                )
        ...
        with patch(
            ...
            with patch.object(
                self.engine.skill_invoker, "invoke_skill", side_effect=mock_invoke_skill
            ):
    ...
    def test_session_management_across_stages(self):
        ...
        with patch(
            ...
            # Verify all stages were recorded (each stage has in_progress + completed)
            completed_stages = [
                s for s in session_data["stages"] if s["status"] == "completed"
            ]
            self.assertEqual(len(completed_stages), 3)

            # Verify status progression
            stage_statuses = [stage["status"] for stage in completed_stages]
    ...
    def test_session_management_with_failures(self):
        ...
        with patch(
            ...
            with patch.object(
                ...
                results = self.engine.execute_workflow(
                    failure_manifest_path, session_id, request_content
                )
            ...
            failed_stages = [
                s for s in session_data["stages"] if s["status"] == "failed"
            ]
            self.assertEqual(len(failed_stages), 1)
            self.assertEqual(failed_stages[0]["status"], "failed")
    ...
    def test_config_overrides_in_workflow(self):
        ...
        def mock_invoke_skill(
            skill_name, context, workspace, config_overrides=None, **kwargs
        ):
            if skill_name == "swe-compliance":
                # Reviewer dispatch - return approval without affecting stage override count
                return SkillInvocationResult(
                    success=True,
                    session_id=f"reviewer-{session_id}",
                    output="Reviewer approved",
            ...
            # Create expected artifacts to pass validation
            self._mock_invoke_with_artifacts(skill_name, context, workspace, **kwargs)
            return SkillInvocationResult(
                success=True,
                session_id=f"skill-{session_id}",
    ...
    def test_artifact_validation_in_workflow(self):
        ...
        def mock_invoke_skill(skill_name, context, workspace, **kwargs):
            ...
            if skill_name == "brainstorming":
                ...
                (Path(workspace) / "test-results.md").write_text(
                    "# Test Results\nAll tests passed."
                )
            elif skill_name == "requesting-code-review":
                (Path(workspace) / "review-findings.md").write_text(
                    "# Review\nNo issues found."
                )
    ...
    def test_artifact_validation_failure_triggers_retry(self):
        ...
        def mock_invoke_skill(skill_name, context, workspace, **kwargs):
            if skill_name == "swe-compliance":
                # Reviewer dispatch - return approval without affecting stage call count
                return SkillInvocationResult(
                    success=True,
                    session_id=f"reviewer-{session_id}",
                    output="Reviewer approved",
        ...
        with patch(
            ...
            with patch.object(
                ...
                with patch("time.sleep"):
                    results = self.engine.execute_workflow(
                        validation_manifest_path, session_id, request_content
                    )
