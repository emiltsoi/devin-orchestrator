...
"""
    ...
    def get_alert_statistics(self) -> dict[str, Any]:
        ...
        recent_24h = len(
            [a for a in alerts if a.timestamp >= datetime.now() - timedelta(hours=24)]
        )
...
class SystemHealthMonitor:
    ...
    def start(self) -> None:
        ...
        self._monitoring_thread = threading.Thread(
            target=self._monitoring_loop, daemon=True, name="SystemHealthMonitor"
        )
    ...
    def _generate_health_alerts(
        self, snapshot: SystemHealthSnapshot, report: dict[str, Any]
    ) -> None:
        """
        ...
        if snapshot.overall_status == "error":
            self.alert_manager.create_alert(
                ...
                message="System health check failed with overall status: error",
                source="SystemHealthMonitor",
                config=self.config,
            ...
            self.alert_manager.create_alert(
                ...
                message="System health check reported warnings",
                source="SystemHealthMonitor",
                config=self.config,
                metadata={"health_report": report},
            )
    ...
    def get_current_health(self) -> SystemHealthSnapshot | None:
        """
...
class WorkflowExecutionMonitor:
    ...
    def monitor_workflow_completion(self, session_id: str) -> None:
        ...
        if workflow_metrics.total_duration:
            ...
            if (
                ...
                > thresholds["workflow_duration_critical"]
            ):
                self.alert_manager.create_alert(
                ...
                workflow_metrics.total_duration
                > thresholds["workflow_duration_warning"]
            ):
                self.alert_manager.create_alert(
                    alert_type=AlertType.PERFORMANCE_DEGRADATION,
                    severity=AlertSeverity.WARNING,
        ...
        for stage_metric in workflow_metrics.stage_metrics:
            if (
                ...
                and stage_metric.duration > thresholds["stage_duration_critical"]
            ):
                self.alert_manager.create_alert(
                    alert_type=AlertType.PERFORMANCE_DEGRADATION,
                    severity=AlertSeverity.CRITICAL,
                ...
                stage_metric.duration
                and stage_metric.duration > thresholds["stage_duration_warning"]
            ):
                self.alert_manager.create_alert(
                    alert_type=AlertType.PERFORMANCE_DEGRADATION,
                    severity=AlertSeverity.WARNING,
    ...
    def check_failure_rates(self, time_window_hours: int = 1) -> None:
        ...
        recent_workflows = [
            m for m in all_metrics.values() if m.start_time >= cutoff_time
        ]
...
class MonitoringSystem:
    ...
    def get_system_health(self) -> SystemHealthSnapshot | None:
    ...
    def get_alerts(
        ...
        return self.alert_manager.get_alerts(
            severity, alert_type, resolved, since, limit
        )
    ...
    def get_monitoring_report(self) -> dict[str, Any]:
        ...
        return {
            ...
            "system_health": {
                ...
                "component_status": health_snapshot.component_status
                if health_snapshot
                else {},
                "active_workflows": health_snapshot.active_workflows
                if health_snapshot
                else 0,
                "recent_failures": health_snapshot.recent_failures
                if health_snapshot
                else 0,
                "resource_metrics": {
                    "cpu_percent": health_snapshot.resource_metrics.cpu_percent
                    if health_snapshot and health_snapshot.resource_metrics
                    else 0,
                    "memory_percent": health_snapshot.resource_metrics.memory_percent
                    if health_snapshot and health_snapshot.resource_metrics
                    else 0,
                    "memory_used_gb": health_snapshot.resource_metrics.memory_used_gb
                    if health_snapshot and health_snapshot.resource_metrics
                    else 0,
                    "memory_total_gb": health_snapshot.resource_metrics.memory_total_gb
                    if health_snapshot and health_snapshot.resource_metrics
                    else 0,
                    "disk_percent": health_snapshot.resource_metrics.disk_percent
                    if health_snapshot and health_snapshot.resource_metrics
                    else 0,
                    "disk_used_gb": health_snapshot.resource_metrics.disk_used_gb
                    if health_snapshot and health_snapshot.resource_metrics
                    else 0,
                    "disk_total_gb": health_snapshot.resource_metrics.disk_total_gb
                    if health_snapshot and health_snapshot.resource_metrics
                    else 0,
                }
                if health_snapshot and health_snapshot.resource_metrics
            ...
            report["health_history"] = [
                {
                    ...
                    "resource_metrics": {
                        "cpu_percent": h.resource_metrics.cpu_percent
                        if h.resource_metrics
                        else 0,
                        "memory_percent": h.resource_metrics.memory_percent
                        if h.resource_metrics
                        else 0,
                        "memory_used_gb": h.resource_metrics.memory_used_gb
                        if h.resource_metrics
                        else 0,
                        "memory_total_gb": h.resource_metrics.memory_total_gb
                        if h.resource_metrics
                        else 0,
                        "disk_percent": h.resource_metrics.disk_percent
                        if h.resource_metrics
                        else 0,
                        "disk_used_gb": h.resource_metrics.disk_used_gb
                        if h.resource_metrics
                        else 0,
                        "disk_total_gb": h.resource_metrics.disk_total_gb
                        if h.resource_metrics
                        else 0,
                    }
                    if h.resource_metrics
