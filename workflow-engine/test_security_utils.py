...
class TestValidatePathSafe:
    ...
    def test_absolute_path_allowed(self):
        """Test that absolute paths inside base are allowed when configured"""
    ...
    def test_empty_target_path_rejected(self):
        """Test that empty target paths are rejected"""
        with tempfile.TemporaryDirectory() as tmpdir:
            base = Path(tmpdir)
            with pytest.raises(InvalidInputError):
                validate_path_safe(base, Path(""))

    def test_null_byte_in_target_rejected(self):
        """Test that null bytes in target paths are rejected"""
        with tempfile.TemporaryDirectory() as tmpdir:
            base = Path(tmpdir)
            with pytest.raises(InvalidInputError):
                validate_path_safe(base, Path(f"{tmpdir}/safe\x00evil"))

    def test_absolute_traversal_outside_base_rejected(self):
        """Test that absolute paths outside base are rejected even with allow_absolute"""
        with tempfile.TemporaryDirectory() as tmpdir:
            base = Path(tmpdir) / "base"
            base.mkdir()
            # An absolute path that resolves outside the base directory
            outside = Path(tmpdir) / "outside"
            outside.mkdir()
            target = outside / "file.txt"
            with pytest.raises(PathTraversalError):
                validate_path_safe(base, target, allow_absolute=True)

    def test_symlink_resolution_does_not_bypass_containment(self):
        """Test that realpath-based resolution prevents symlink containment bypass"""
        if platform.system() == "Windows":
            pytest.skip("Symlink creation requires elevated privileges on Windows")
        with tempfile.TemporaryDirectory() as tmpdir:
            base = Path(tmpdir) / "safe"
            ...
            inside = base / "real"
            inside.mkdir()
            # Symlink inside base pointing to a directory also inside base is OK
            symlink = base / "link"
            symlink.symlink_to(inside)
            target = symlink / "file.txt"
            result = validate_path_safe(base, target)
            # realpath should resolve the symlink back into the base
...
class TestSanitizeFilename:
    ...
    def test_double_dot_sequence_collapse(self):
        """Test that sequences of dots cannot collapse back into traversal"""
        # "...." should be fully stripped, not collapse into ".."
        # After repeated ".." stripping and trailing-dot strip, must be empty
        with pytest.raises(InvalidInputError):
            sanitize_filename("....")

    def test_double_dot_with_text_stripped(self):
        """Test that '..' segments embedded in text are removed robustly"""
        result = sanitize_filename("a..b..c")
        assert ".." not in result
        assert result == "abc"

    def test_leading_trailing_dots_stripped(self):
        """Test that leading/trailing dots are stripped so no hidden segments remain"""
        result = sanitize_filename(".hidden.")
        assert not result.startswith(".")
        assert not result.endswith(".")

    def test_resolves_to_empty_raises(self):
        """Test that a filename resolving to empty string raises error"""
        with pytest.raises(InvalidInputError):
            sanitize_filename("/\\..")
...
class TestValidateSessionId:
    ...
    def test_leading_trailing_dot_rejected(self):
        """Test that leading/trailing dots are rejected"""
        with pytest.raises(InvalidInputError):
            validate_session_id(".SESSION-123")
        with pytest.raises(InvalidInputError):
            validate_session_id("SESSION-123.")
    ...
    def test_control_characters_rejected(self):
        ...
        with pytest.raises(InvalidInputError):
            validate_session_id("SESSION\x01-123")
        with pytest.raises(InvalidInputError):
            validate_session_id("SESSION-123\x7f")
...
class TestValidateSkillName:
    ...
    def test_leading_trailing_whitespace_rejected(self):
        """Test that leading/trailing whitespace is rejected"""
        with pytest.raises(InvalidInputError):
            validate_skill_name(" brainstorming")
        with pytest.raises(InvalidInputError):
            validate_skill_name("brainstorming ")
    ...
    def test_control_characters_rejected(self):
        """Test that control characters are rejected"""
        with pytest.raises(InvalidInputError):
            validate_skill_name("brain\x01storming")
        with pytest.raises(InvalidInputError):
            validate_skill_name("brainstorming\x7f")
...
class TestValidateWorkspacePath:
    ...
    def test_valid_workspace(self):
        """Test that valid workspace is accepted (base_allowed_dir required)"""
        with tempfile.TemporaryDirectory() as tmpdir:
            base = Path(tmpdir) / "base"
            base.mkdir()
    ...
    def test_workspace_without_base_rejected(self):
        """Test that workspace validation requires base_allowed_dir"""
        with tempfile.TemporaryDirectory() as tmpdir:
            with pytest.raises(InvalidInputError):
                validate_workspace_path(tmpdir)
    ...
    def test_workspace_traversal_escape_rejected(self):
        """Test that traversal in workspace path is rejected"""
        with tempfile.TemporaryDirectory() as tmpdir:
            base = Path(tmpdir) / "base"
            base.mkdir()
            # Attempt to escape via traversal
            escape = str(base / ".." / "outside")
            with pytest.raises(PathTraversalError):
                validate_workspace_path(escape, base_allowed_dir=base)

    def test_workspace_null_byte_rejected(self):
        """Test that null bytes in workspace path are rejected"""
        with tempfile.TemporaryDirectory() as tmpdir:
            base = Path(tmpdir) / "base"
            base.mkdir()
            with pytest.raises(InvalidInputError):
