...
class TestGateController(unittest.TestCase):
    ...
    def test_bypass_mandatory_gate(self):
        ...
        manifest = {"gates": [{"id": "g1", "mandatory": True}]}
        result = self.controller.evaluate_gate_bypass_conditions(
            "g1", "stage", self.session_dir, self._decision_path(), manifest, {"success": True}
        )
    ...
    def test_bypass_stage_failure(self):
        ...
        result = self.controller.evaluate_gate_bypass_conditions(
            "g1", "stage", self.session_dir, self._decision_path(), None, {"success": False}
        )
        self.assertEqual(result["verdict"], "block")
    ...
    def test_bypass_benign_keywords_ignored(self):
        ...
        result = self.controller.evaluate_gate_bypass_conditions(
            "g1", "stage", self.session_dir, self._decision_path(), None,
            {"success": True, "reviewer_verdict": "PASS", "confidence": "HIGH", "output": "critical security block"},
        )
        self.assertEqual(result["verdict"], "approve")
    ...
    def test_bypass_empty_output(self):
        ...
        result = self.controller.evaluate_gate_bypass_conditions(
            "g1", "stage", self.session_dir, self._decision_path(), None,
            {"success": True, "reviewer_verdict": "PASS", "confidence": "HIGH", "output": ""},
        )
        self.assertEqual(result["verdict"], "request_changes")
    ...
    def test_bypass_high_confidence_with_block_condition(self):
        ...
        manifest = {"gates": [{"id": "g1", "mandatory": True}]}
        result = self.controller.evaluate_gate_bypass_conditions(
            "g1", "stage", self.session_dir, self._decision_path(), manifest,
    ...
    def test_create_gate_decision_file_writes_template(self):
        ...
        self.engine.artifact_validator.validate_artifact_path.return_value = self._decision_path()

        error = self.controller.create_gate_decision_file("g1", "stage", self.session_dir)
    ...
    def test_handle_gate_auto_approve(self):
        ...
        with patch("gate_controller.record_gate") as mock_record:
            result = self.controller.handle_gate("g1", "stage", self.session_dir)
    ...
    def test_handle_gate_auto_request_changes(self):
        ...
        result = self.controller.handle_gate(
            "g1", "stage", self.session_dir, stage_result={"success": True, "output": ""}
        )
    ...
    @patch("gate_controller.wait_for_file_change", return_value=False)
    @patch("gate_controller.record_gate")
    def test_wait_and_parse_gate_decision_approve(self, mock_record, mock_wait):
        ...
        result = self.controller.wait_and_parse_gate_decision("g1", self.session_dir, path)
    ...
    def test_wait_and_parse_gate_decision_timeout(self, mock_record, mock_wait):
        ...
        self.assertTrue(result["blocked"])
        mock_record.assert_called_once_with("g1", "block", self.session_dir, unittest.mock.ANY)

    # ---------- build_gate_signal ----------

    def test_build_gate_signal_contains_required_keys(self):
        path = self._decision_path()
        signal = self.controller.build_gate_signal("g1", "stage", self.session_dir, path)
