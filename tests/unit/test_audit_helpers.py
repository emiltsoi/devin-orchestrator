...
from devin_orchestrator.audit_helpers import append_audit, record_gate, write_run_jsonl
...
class TestRecordGate:
    ...
    def test_record_gate_multiple_entries(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            record_gate(
                session_dir=session_dir, gate_id="g2_plan_approval", verdict="approved"
            )
...
class TestConcurrentWrites:
    ...
    def test_concurrent_audit_writes(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            threads = [
                threading.Thread(target=write_audit, args=(i,)) for i in range(10)
            ]
            for t in threads:
                t.start()
            for t in threads:
    ...
    def test_concurrent_jsonl_writes(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            threads = [
                threading.Thread(target=write_jsonl, args=(i,)) for i in range(10)
            ]
            for t in threads:
                t.start()
            for t in threads:
...
class TestAppendAuditEdgeCases:
    """Edge cases for append_audit"""

    def test_append_audit_special_characters_in_rationale(self):
        """Special characters in rationale should be written verbatim."""
        with tempfile.TemporaryDirectory() as tmpdir:
            session_dir = Path(tmpdir)
            append_audit(
                session_dir=session_dir,
                ...
                reviewer_verdict="PASS",
                confidence="HIGH",
                rationale="Handles <html> & 'quotes' \"too\"",
                triage_decision="proceed",
                retry_count=0,
                gate_verdict="none",
            )
            content = (session_dir / "session-audit.md").read_text()
            assert "<html>" in content
            assert '"too"' in content

    def test_append_audit_multiple_context_items(self):
        """Multiple injected context items should be comma-joined."""
        with tempfile.TemporaryDirectory() as tmpdir:
            session_dir = Path(tmpdir)
            append_audit(
                session_dir=session_dir,
                stage="step_1",
                skill="brainstorming",
                injected_context=["design.md", "plan.md", "spec.md"],
                structural_result="PASS",
                reviewer_verdict="PASS",
                confidence="HIGH",
                rationale="Good",
            ...
            content = (session_dir / "session-audit.md").read_text()
            assert "design.md, plan.md, spec.md" in content

    def test_append_audit_fail_result(self):
        """FAIL structural result should be recorded verbatim."""
        with tempfile.TemporaryDirectory() as tmpdir:
            session_dir = Path(tmpdir)
            append_audit(
                session_dir=session_dir,
                ...
                triage_decision="correct",
                retry_count=2,
                gate_verdict="rejected",
            )
            content = (session_dir / "session-audit.md").read_text()
            assert "Structural Result: FAIL" in content
            assert "Retry Count: 2" in content
            assert "Gate Verdict: rejected" in content


class TestRecordGateEdgeCases:
    """Edge cases for record_gate"""

    def test_record_gate_appends_to_existing_audit(self):
        """record_gate should append to an existing audit file."""
        with tempfile.TemporaryDirectory() as tmpdir:
            session_dir = Path(tmpdir)
            append_audit(
                session_dir=session_dir,
            ...
            record_gate(session_dir=session_dir, gate_id="g1", verdict="approved")
            content = (session_dir / "session-audit.md").read_text()
            assert "## Stage: step_1" in content
            assert "## Gate Decision: g1" in content

    def test_record_gate_verdicts(self):
        """All verdict strings should be written verbatim."""
        with tempfile.TemporaryDirectory() as tmpdir:
            session_dir = Path(tmpdir)
            for v in ("approved", "rejected", "needs_info"):
                record_gate(session_dir=session_dir, gate_id=f"g_{v}", verdict=v)
            content = (session_dir / "session-audit.md").read_text()
            for v in ("approved", "rejected", "needs_info"):
                assert f"Verdict: {v}" in content


class TestWriteRunJsonlEdgeCases:
    """Edge cases for write_run_jsonl"""

    def test_write_run_jsonl_missing_multiple_fields(self):
        """Missing multiple required fields should raise with all listed."""
        with tempfile.TemporaryDirectory() as tmpdir:
            session_dir = Path(tmpdir)
            entry = {"timestamp": "now", "session_id": "s1"}
            with pytest.raises(ValueError) as exc_info:
                write_run_jsonl(session_dir, entry)
            message = str(exc_info.value)
            # The validator reports the first missing field only.
            assert "Missing required field" in message

    def test_write_run_jsonl_extra_fields_preserved(self):
        """Extra non-required fields should be preserved in the JSONL entry."""
        with tempfile.TemporaryDirectory() as tmpdir:
            session_dir = Path(tmpdir)
            entry = {
                "timestamp": datetime.now(timezone.utc).isoformat(),
            ...
            write_run_jsonl(session_dir, entry)
            parsed = json.loads((session_dir / "run.jsonl").read_text().strip())
            assert parsed["custom_field"] == "custom_value"

    def test_write_run_jsonl_creates_parent_dir(self):
        """write_run_jsonl should fail if session_dir doesn't exist."""
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # The helper does not create parent dirs; FileNotFoundError is expected.
