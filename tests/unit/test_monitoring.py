...
from devin_orchestrator.metrics import MetricsCollector, WorkflowMetrics
from devin_orchestrator.monitoring import (
    AlertManager,
    AlertSeverity,
    AlertType,
    MonitoringConfig,
...
def test_resource_alerts():
    ...
    print(f"  Generated {len(resource_alerts)} resource alerts")
...
def test_workflow_execution_monitor():
    ...
    workflow_monitor = WorkflowExecutionMonitor(
        config, alert_manager, metrics_collector
    )
...
# ---------------------------------------------------------------------------
# Edge-case tests (additive; do not modify existing tests above)
# ---------------------------------------------------------------------------


def test_resolve_alert_unknown_id_returns_false():
    """resolve_alert should return False for an unknown alert id."""
    config = MonitoringConfig()
    alert_manager = AlertManager(config)
    assert alert_manager.resolve_alert("does-not-exist") is False


def test_resolve_alert_already_resolved_returns_false():
    """resolve_alert should return False when the alert is already resolved."""
    config = MonitoringConfig()
    alert_manager = AlertManager(config)
    alert = alert_manager.create_alert(
        alert_type=AlertType.SYSTEM_HEALTH,
    ...
    assert alert_manager.resolve_alert(alert.alert_id) is True
    # Second resolution should be a no-op.
    assert alert_manager.resolve_alert(alert.alert_id) is False


def test_get_alerts_filters_by_resolved_status():
    """get_alerts should filter by resolved status when requested."""
    config = MonitoringConfig()
    alert_manager = AlertManager(config)
    ...
    alert_manager.resolve_alert(a1.alert_id)
    unresolved = alert_manager.get_alerts(resolved=False)
    resolved = alert_manager.get_alerts(resolved=True)
    assert len(unresolved) == 1
    assert len(resolved) == 1
    assert resolved[0].alert_id == a1.alert_id


def test_get_alerts_combined_severity_and_type_filter():
    """get_alerts should AND multiple filters together."""
    config = MonitoringConfig()
    alert_manager = AlertManager(config)
    alert_manager.create_alert(
        alert_type=AlertType.SYSTEM_HEALTH,
    ...
    filtered = alert_manager.get_alerts(
        severity=AlertSeverity.ERROR, alert_type=AlertType.SYSTEM_HEALTH
    )
    assert len(filtered) == 1
    assert filtered[0].title == "t1"


def test_get_alerts_since_filter():
    """get_alerts should only return alerts at or after the `since` timestamp."""
    ...
    # `since` in the future should exclude the alert.
    future = _datetime.now() + timedelta(hours=1)
    assert alert_manager.get_alerts(since=future) == []
    # `since` well in the past should include it.
    past = _datetime.now() - timedelta(hours=1)
    assert len(alert_manager.get_alerts(since=past)) == 1


def test_get_alerts_respects_limit():
    """get_alerts should cap the number of returned alerts at `limit`."""
    config = MonitoringConfig()
    alert_manager = AlertManager(config)
    for i in range(5):
        alert_manager.create_alert(
    ...
    assert len(alert_manager.get_alerts(limit=2)) == 2


def test_get_alert_statistics_by_severity_and_type():
    """get_alert_statistics should bucket counts by severity and type."""
    config = MonitoringConfig()
    alert_manager = AlertManager(config)
    alert_manager.create_alert(
        alert_type=AlertType.SYSTEM_HEALTH,
    ...
    stats = alert_manager.get_alert_statistics()
    assert stats["total_alerts"] == 1
    assert stats["by_severity"][AlertSeverity.ERROR.value] == 1
    assert stats["by_type"][AlertType.SYSTEM_HEALTH.value] == 1
    assert "recent_24h" in stats


def test_monitoring_system_resolve_alert_unknown_returns_false():
    """MonitoringSystem.resolve_alert should return False for unknown ids."""
    reset_monitoring_system()
    try:
        monitoring = MonitoringSystem(MonitoringConfig())
        assert monitoring.resolve_alert("nope") is False
    finally:
        reset_monitoring_system()


def test_monitoring_system_check_failure_rates_no_raise():
    """MonitoringSystem.check_failure_rates should not raise with no data."""
    reset_monitoring_system()
    try:
        monitoring = MonitoringSystem(MonitoringConfig())
...
def test_monitoring_system_monitor_workflow_no_raise():
    """MonitoringSystem.monitor_workflow should not raise for unknown session."""
    reset_monitoring_system()
    try:
        monitoring = MonitoringSystem(MonitoringConfig())
        monitoring.monitor_workflow("unknown-session")
    finally:
        reset_monitoring_system()


def test_monitoring_system_start_stop_idempotent():
    """MonitoringSystem.start/stop should be safe to call repeatedly."""
    reset_monitoring_system()
    try:
        monitoring = MonitoringSystem(MonitoringConfig())
        monitoring.start()
        monitoring.start()  # second start should not raise
        monitoring.stop()
        monitoring.stop()  # second stop should not raise
    finally:
        reset_monitoring_system()


def test_get_health_history_respects_limit():
    """get_health_history should cap returned snapshots at `limit`."""
    config = MonitoringConfig()
    alert_manager = AlertManager(config)
    health_monitor = SystemHealthMonitor(config, alert_manager)
    # Run several checks to populate history.
    for _ in range(5):
        health_monitor._check_health()
    limited = health_monitor.get_health_history(limit=2)
    assert len(limited) <= 2


def test_get_monitoring_report_contains_expected_keys():
    """get_monitoring_report should include the documented top-level keys."""
    reset_monitoring_system()
    try:
        monitoring = MonitoringSystem(MonitoringConfig())
        report = monitoring.get_monitoring_report()
        for key in ("timestamp", "alerts", "configuration"):
            assert key in report
    finally:
        reset_monitoring_system()


def test_export_monitoring_data_failure_returns_false(tmp_path):
    """export_monitoring_data should return False when the file write fails."""
    reset_monitoring_system()
    try:
        monitoring = MonitoringSystem(MonitoringConfig())
        bad_path = tmp_path / "nonexistent_dir" / "out.json"
        # Parent dir doesn't exist and export doesn't create it -> failure.
        # Note: if export creates parent dirs, this test would need adjustment;
        # we patch open to force an OSError instead.
        from unittest.mock import patch

        with patch("builtins.open", side_effect=OSError("denied")):
            result = monitoring.export_monitoring_data(tmp_path / "out.json")
