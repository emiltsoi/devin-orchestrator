...
For a concrete, canonical example, see `workflows/superpower.runbook.md`. It is the prose companion to `workflows/superpower.manifest.yaml` and uses the canonical stage numbering defined there:
...
This example is intentionally not duplicated here so that stage names and numbers have a single source of truth.
