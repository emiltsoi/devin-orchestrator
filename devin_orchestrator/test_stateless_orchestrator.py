...
def test_run_workflow_async_starts_and_writes_result(orchestrator, tmp_path):
    ...
    with patch("stateless_orchestrator.OrchestrationEngine.execute_workflow") as mock_exec:
...
def test_run_skill_async_starts_and_writes_result(orchestrator):
    ...
    with patch("stateless_orchestrator.SkillInvoker.invoke_skill") as mock_invoke:
...
def test_continue_workflow_async_starts_and_writes_result(orchestrator, tmp_path):
    ...
    with patch(
        "stateless_orchestrator.OrchestrationEngine.continue_workflow"
    ) as mock_continue:
...
def test_execute_async_routes_to_implement(orchestrator, tmp_path):
    ...
    with patch("stateless_orchestrator.OrchestrationEngine.execute_workflow") as mock_exec:
        ...
        result = orchestrator.execute_async("Add a new API endpoint", intent="implement")
